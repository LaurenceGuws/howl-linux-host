...
fn drawSpriteCommand(surface: render_c.HowlRenderHostSurface, command: render_c.HowlRenderSurfaceCommand, slot: RenderResourceTextures.Slot) void {
    ...
    const source_rect = render_c.HowlRenderSurfaceRect{ .x_px = 0, .y_px = 0, .width_px = command.rect.width_px, .height_px = command.rect.height_px };
...
fn drawGlyphCommand(textures: *RenderResourceTextures, surface: render_c.HowlRenderHostSurface, command: render_c.HowlRenderSurfaceCommand) void {
    ...
    for (glyphs) |glyph| {
        ...
        gl_c.glColor4ub(rgba[0], rgba[1], rgba[2], rgba[3]);
        const rect = render_c.HowlRenderSurfaceRect{ .x_px = glyph.x_px, .y_px = glyph.y_px, .width_px = glyph.atlas_rect.width_px, .height_px = glyph.atlas_rect.height_px };
...
pub const testing = struct {
    ...
    pub fn ndcY(y: i32, height: u16) f32 {
        return @import("render_surface_commands.zig").ndcY(y, height);
    }

    pub fn fillUploadRowsPerChunk(width: u16, height: u16) u16 {
        return @import("render_surface_commands.zig").fillUploadRowsPerChunk(width, height);
    }
    ...
    pub fn spriteUploadCoversCommand(slot: RenderResourceTextures.Slot, command_rect: render_c.HowlRenderSurfaceRect) bool {
        return @import("render_surface_commands.zig").spriteUploadCoversCommand(slot, command_rect);
    }
};
