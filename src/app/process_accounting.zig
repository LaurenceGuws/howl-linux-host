...
pub const State = struct {
    ...
    pub fn countWaitAdmission(self: *State, admission: WaitAdmission) void {
        ...
        if (admission.wait) {
            ...
            if (admission.present_complete_pending) {
                self.counters.wait_false_present_complete_pending += 1;
            }
        ...
        if (admission.render_permission) {
            ...
            if (admission.render_work_pending) {
                self.counters.render_permission_render_work_pending += 1;
            }
        }
    }
    ...
    pub fn maybeLog(
        ...
        if (self.has_previous) {
            const interval_ns = if (now_ns > self.previous_sample_ns)
...
pub fn parseStat(contents: []const u8, tid: u32) !ThreadStat {
    ...
    stat.ticks = (utime orelse return error.InvalidProcStat) +
        (stime orelse return error.InvalidProcStat);
    return stat;
}
...
pub fn formatThreadDelta(
    ...
    return formatThreadDeltaWithClock(
        ...
        current,
        interval_ns,
        clock_ticks_per_second_fallback,
    );
}
...
fn formatThreadDeltaWithClock(
    ...
    return std.fmt.bufPrint(
        buffer,
        "tid={} name={s} cpu_milli={} ticks={}",
        .{ current.tid, current.nameSlice(), cpu_milli, delta_ticks },
    );
}
...
fn logSample(sample: Sample, turn_count: u64, intent: RuntimeIntent, counters: Counters) void {
    ...
    std.debug.print(
        "howl-debug turn={} interval_ms={} process_cpu_milli={} process_ticks={} " ++
            "wake_count={} runtime_due_count={} render_pending_count={} " ++
            "host_redraw={} terminal_redraw={} loop_turns={}\n",
        .{
            turn_count,
    ...
    std.debug.print(
        "howl-debug counters wait_true={} wait_false={} " ++
            "wait_false_owner_work={} wait_false_runtime_admission={} " ++
            "wait_false_runtime_wake={} wait_false_present_complete_pending={} " ++
            "wait_false_render_permission={} render_permission_redraw_requested={} " ++
            "render_permission_render_work_pending={} terminal_keep={} " ++
            "terminal_should_redraw={} terminal_drive_performed={} " ++
            "render_step_surface_idle={} " ++
            "render_step_idle_prepare={} render_step_idle_submit={} " ++
            "render_step_blocked_present={} render_step_rendered={} render_step_failed={} " ++
            "present_submitted={} present_skipped={} present_complete_drained={} " ++
            "present_reason_none={} present_reason_host_damage={} " ++
            "present_reason_terminal_frame={} present_reason_terminal_retire={} " ++
...
fn logThreads(sample: Sample) void {
    ...
    for (sample.current.threads[0..sample.current.thread_count]) |current| {
        ...
        const line = formatThreadDeltaWithClock(
            &buffer,
            previous,
            current,
            sample.interval_ns,
...
test "format thread delta reports tid name and ticks" {
    ...
    try std.testing.expectEqualStrings(
        "tid=7 name=howl-term-host cpu_milli=50 ticks=5",
        line,
    );
}
