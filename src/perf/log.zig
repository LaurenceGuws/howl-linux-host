//! Responsibility: own Linux host runtime performance logging.
//! Ownership: SDL present-rate sampling plus /proc thread CPU snapshots.
//! Reason: keeps iteration-time performance diagnostics out of the host runtime loop.
...
const log_name = "howl-thread-cpu.jsonl";
...
pub const State = struct {
    ...
    pub fn init(self: *State, term: *api.Term) !void {
        const file = c.fopen(log_name, "w") orelse return error.PerfLogOpenFailed;
...
fn sample(self: *State, prev_threads: *std.ArrayList(ThreadPrev), last_sample_ns: *u64, last_present_count: *u64, now_ns: u64) !void {
    ...
    if (c.fprintf(file, "{\"type\":\"perf\",\"schema\":1,\"mono_ns\":%llu,\"elapsed_ns\":%llu,\"fps\":%.2f,\"prepare\":{\"term_us\":%llu,\"sync_us\":%llu,\"copy_us\":%llu,\"renderer_us\":%llu,\"input_us\":%llu,\"sparse_us\":%llu,\"clusters_us\":%llu,\"resolve_us\":%llu,\"shape_us\":%llu,\"group_us\":%llu,\"scene_us\":%llu,\"raster_us\":%llu,\"atlas_us\":%llu},\"queue\":{\"prepare_requests\":%llu,\"prepare_coalesces\":%llu,\"prepare_takes\":%llu,\"prepared_publishes\":%llu,\"submit_takes\":%llu,\"submit_valid\":%llu,\"submitted_accepts\":%llu,\"presents\":%llu},\"render\":{\"shape_requests\":%llu,\"shape_cache_hits\":%llu,\"face_checks\":%llu,\"face_cache_hits\":%llu,\"fallback_hits\":%llu,\"fallback_misses\":%llu,\"glyphs\":%zu,\"uploads\":%zu},\"threads\":[",
        ...
        render_metrics.glyphs,
        render_metrics.uploads,
    ) < 0) {
        return error.PerfLogWriteFailed;
    }
    for (threads.items, 0..) |thread, idx| {
        ...
        if (c.fprintf(file, "%s{\"tid\":%u,\"name\":\"%.*s\",\"cpu\":%.2f}", comma, @as(c_uint, thread.tid), @as(c_int, @intCast(thread.name_len)), &thread.name[0], cpu_percent) < 0) {
            return error.PerfLogWriteFailed;
        }
    }
...
fn previousTicks(prev: []const ThreadPrev, tid: u32) ?u64 {
    for (prev) |entry| {
        if (entry.tid == tid) return entry.total_ticks;
    }
    return null;
}
