...
const DisplayViewport = @import("display/viewport.zig");
...
pub const Processor = struct {
    ...
    pub fn openTab(self: *Self) !void {
        ...
        const before_tab_bar_height = DisplayViewport.tabBarHeight(self.conf.tab_bar, items.len);
        const next_viewport = DisplayViewport.regions(self.window, self.conf.tab_bar, items.len + 1);
    ...
    fn forwardTerminalInput(self: *Self) TerminalSurface.DrainInputOutcome {
        const tab = activeSurface(self.tabs.items(), self.active_tab_idx.*);
        const viewport = DisplayViewport.terminal(self.window, self.conf.tab_bar, self.tabs.items().len, tab.textureSize());
    ...
    fn resizeTerminals(conf: *const Config.UiConfig, app_window: *window.Window, tabs: []*TerminalSurface) void {
        resizeTerminalsForViewport(tabs, DisplayViewport.regions(app_window, conf.tab_bar, tabs.len));
    }

    fn resizeTerminalsForViewport(tabs: []*TerminalSurface, viewport: DisplayViewport.Regions) void {
    ...
    fn closeActiveTab(conf: *const Config.UiConfig, app_window: *window.Window, tabs: *TabSlots, active_tab_idx: *TabIndex) void {
        ...
        const before_tab_bar_height = DisplayViewport.tabBarHeight(conf.tab_bar, items.len);
        ...
        const after_viewport = DisplayViewport.regions(app_window, conf.tab_bar, updated.len);
...
test "tab bar height follows configured minimum tab count" {
    ...
    try std.testing.expectEqual(@as(u32, 0), DisplayViewport.tabBarHeight(tab_bar, 0));
    try std.testing.expectEqual(@as(u32, 0), DisplayViewport.tabBarHeight(tab_bar, 1));
    try std.testing.expectEqual(@as(u32, 30), DisplayViewport.tabBarHeight(tab_bar, 2));
    try std.testing.expectEqual(@as(u32, 30), DisplayViewport.tabBarHeight(tab_bar, 3));
}
