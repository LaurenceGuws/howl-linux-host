...
pub fn handleMouse(context: anytype, mouse_event: HostInput.Mouse.Event) MouseHandlingOutcome {
    switch (mouse_event.kind) {
        .press => {
            ...
            context.selection_anchor = eventCell(context, mouse_event);
            context.selection_drag_active = false;
            return .{ .consumed = true, .host_visual_changed = false };
        },
        .move => {
            if (context.selection_anchor == null or !mouse_event.buttons_down.left) {
                return .{ .consumed = false, .host_visual_changed = false };
            }
            const anchor = context.selection_anchor.?;
            const cell = eventCell(context, mouse_event);
            if (!context.selection_drag_active) {
                ...
                context.selection_drag_active = true;
            }
        ...
        .release => {
            ...
            if (context.selection_anchor == null) return .{ .consumed = false, .host_visual_changed = false };
            if (!context.selection_drag_active) {
                context.selection_anchor = null;
                return .{ .consumed = true, .host_visual_changed = false };
            }
            const cell = eventCell(context, mouse_event);
            vt_retained.updateSelection(&context.term, cell.row, cell.col) catch {
