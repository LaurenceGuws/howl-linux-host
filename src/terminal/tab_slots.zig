...
pub const Slots = struct {
    ...
    pub fn items(self: *Slots) []*TerminalContext {
        self.assertCounts();
        return self.active_tabs[0..self.active_count];
    }

    pub fn acquireSlot(self: *Slots) ?struct { slot_idx: TabIndex, tab: *TerminalContext } {
        self.assertCounts();
    ...
    pub fn appendActive(self: *Slots, slot_idx: TabIndex, tab: *TerminalContext) void {
        self.assertCounts();
    ...
    pub fn orderedRemoveActive(self: *Slots, idx: TabIndex) struct { slot_idx: TabIndex, tab: *TerminalContext } {
        self.assertCounts();
...
test "tab slots preserve order on close semantics" {
    var tabs = Slots.init();
    var active: [4]*TerminalContext = undefined;
